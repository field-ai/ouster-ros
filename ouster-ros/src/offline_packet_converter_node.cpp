...
bool OfflinePacketConverterNode::process() {
  ...
  // NOTE: construct with SensorInfo (not the (w, h, profile) overload) so that
  // the RGB profile's channels are laid out the same way the live driver lays
  // them out (see lidar_packet_handler.h / rgb_tonemap.h).
  ouster::sdk::core::LidarScan scan(ouster_metadata_);

  // Derives R8/G8/B8 from the batched 16-bit R/G/B for RGB16 profiles so that
  // point_type:=color_point (and native RGB types) get populated color.
  // No-op for non-RGB profiles.
  ouster_ros::RgbTonemapper rgb_tonemapper;
  rgb_tonemapper.setup(scan, ouster_metadata_);
